...
const { notifyUser } = require("./WalletSSE"); // ← SSE push helper
...
const addMoney = async (req, res) => {
    ...
    try {
        ...
        notifyUser(req.params.userId, { newBalance: wallet.balance, lockedBalance: wallet.lockedBalance });
...
const debitWallet = async (req, res) => {
    const { amount, label, category, referenceId, paymentId } = req.body;
    ...
    try {
        ...
        // Persist winner payment state so WonAuctions survives refresh.
        if (category === "auction_won" && referenceId) {
            try {
                await AuctionResult.findOneAndUpdate(
                    { auction: referenceId, winner: req.params.userId },
                    {
                        $set: {
                            paymentStatus: "Paid",
                            paymentMethod: paymentId ? "Razorpay" : "Wallet",
                            paymentId: paymentId || "",
                            paidAt: new Date(),
                        },
                    },
                ...
                console.error("debitWallet mark paid error:", markErr.message);
            }
        }
...
const releaseDepositsForAuction = async ({ auctionId, winnerUserId, auctionTitle }) => {
    ...
    for (const wallet of wallets) {
        ...
        // ── Notify connected client ──
        notifyUser(String(wallet.userId), { newBalance: wallet.balance, lockedBalance: wallet.lockedBalance });
